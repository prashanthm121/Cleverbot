	...
	public class CleverbotResponse
	{
		...
		*/
		...
		internal static async Task<CleverbotResponse> CreateAsync(string message, string conversationId, string apiKey)
		{
			...
			var response = await HttpWebRequest.Create($"https://www.cleverbot.com/getreply?key={ apiKey }&wrapper=cleverbot.net&input={ message }{ conversationLine }").GetResponseAsync();
			...
			CleverbotResponse cResponse = JsonConvert.DeserializeObject<CleverbotResponse>(responseString);
			cResponse.apiKey = apiKey;
			cResponse.CreateInteractionsList();
